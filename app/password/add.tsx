      ...
      <View className="flex-1" style={{backgroundColor: '#e5e7eb'}}>
        ...
        <View className="bg-[#f0f0f3] px-5 py-2" style={{borderBottomWidth: 1, borderBottomColor: '#d4d7d9'}}>
          <View className="flex-row items-center justify-between">
            ...
            <Text className="text-xl font-semibold text-[#1a1a2e]">添加密码</Text>
            <View className="w-8"/>
          </View>
        </View>
        ...
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} className="flex-1">
              ...
              <View>
                ...
                <View className="h-full absolute right-3 flex-row items-center">
                  <TouchableOpacity onPress={() => setShowPassword(!showPassword)}>
                    {showPassword ? <EyeOff size={22} className="text-[#6b7280]"/> :
              ...
              {password.length > 0 && (
                  <View className="mt-3">
                    <View className="h-2 bg-[#e5e7eb] rounded-full overflow-hidden flex-row">
                      {[0, 1, 2, 3, 4].map((i) => (
                          <View key={i} className="flex-1 h-full"
                ...
                <View className="mb-3">
                  ...
                  <View className="flex-row gap-1">
                    {[8, 12, 16, 20, 24].map((length) => (
                        ...
                        >
                          <Text
                              className={`text-xs text-center leading-7 ${passwordLength === length ? 'text-white font-semibold' : 'text-[#1a1a2e]'}`}>
                            {length}
                          </Text>
                        </TouchableOpacity>
                    ))}
                ...
                <View style={{gap: 8}}>
                  {[
                    ...
                    {key: 'symbols', label: '符号 (!@#$...)'},
                      ...
                      <TouchableOpacity
                          key={option.key}
                          onPress={() => setPasswordOptions({
                            ...passwordOptions,
                            [option.key]: !passwordOptions[option.key as keyof PasswordOptions]
                          })}
                          className="flex-row items-center py-2"
                      >
                        <View
                            className={`w-6 h-6 rounded-lg border-2 mr-3 items-center justify-center ${passwordOptions[option.key as keyof PasswordOptions] ? 'bg-[#6366f1] border-[#6366f1]' : 'border-[#d4d7d9]'}`}>
                          {passwordOptions[option.key as keyof PasswordOptions] &&
          ...
          <View className="px-5 py-5">
            <TouchableOpacity
                onPress={handleSave}
                disabled={isLoading || !platform || !username || !password}
                className={`rounded-2xl py-4 px-6 items-center ${isLoading || !platform || !username || !password ? 'bg-[#d4d7d9]' : 'bg-[#6366f1]'}`}
                style={{
                  shadowColor: '#000',
                  shadowOffset: {width: 0, height: 4},
                  shadowOpacity: isLoading || !platform || !username || !password ? 0.1 : 0.25,
                  shadowRadius: 8,
                  elevation: 3,
                }}
            >
              <Text
                  className={`font-semibold text-base ${isLoading || !platform || !username || !password ? 'text-[#6b7280]' : 'text-white'}`}>
                {isLoading ? '保存中...' : '保存'}
              </Text>
            </TouchableOpacity>
          </View>
